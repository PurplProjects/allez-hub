...
// The eventSchedule HTML page loads events via JS so static fetch returns nothing.
// Instead we try several known JSON data endpoints, then fall back to HTML parsing.
async function getEventGUIDs(tournamentGUID) {
  // Try 1: /tournaments/schedule/data/{guid} — likely JSON endpoint
  try {
    const data = await getJSON(`${FTL}/tournaments/schedule/data/${tournamentGUID}`);
    if (Array.isArray(data) && data.length > 0) {
      const guids = data.map(e => e.id || e.eventId || e.guid).filter(Boolean);
      if (guids.length > 0) return guids;
    }
  } catch { /* try next */ }

  // Try 2: /events/schedule/data/{guid}
  try {
    const data = await getJSON(`${FTL}/events/schedule/data/${tournamentGUID}`);
    if (Array.isArray(data) && data.length > 0) {
      const guids = data.map(e => e.id || e.eventId || e.guid).filter(Boolean);
      if (guids.length > 0) return guids;
    }
  ...
  // Try 3: Puppeteer — render the JS page and read event links from DOM
  let browser;
  try {
    ...
    // Intercept network requests to catch the JSON data call
    const capturedGUIDs = [];
    page.on('response', async (response) => {
      const url = response.url();
      if (url.includes('/events/results/data/') || url.includes('/schedule/data/')) {
        try {
          const json = await response.json();
          if (Array.isArray(json)) {
            json.forEach(item => {
              const guid = item.id || item.eventId;
              if (guid && /^[A-F0-9]{32}$/i.test(guid)) capturedGUIDs.push(guid);
            });
          }
        } catch { /* not JSON */ }
      }
    ...
    await page.goto(`${FTL}/tournaments/eventSchedule/${tournamentGUID}`, {
      waitUntil: 'networkidle0', timeout: 20000,
    });
    await new Promise(r => setTimeout(r, 2000));

    // Also read from DOM — look for links to /events/results/
    const domGUIDs = await page.evaluate(() => {
      const links = [...document.querySelectorAll('a[href*="/events/results/"]')];
      return links.map(a => {
        const m = a.href.match(/\/events\/results\/([A-Fa-f0-9]{32})/);
        return m ? m[1].toUpperCase() : null;
      }).filter(Boolean);
    });

    const allGUIDs = [...new Set([...capturedGUIDs.map(g => g.toUpperCase()), ...domGUIDs])];
    return allGUIDs;

  } catch (err) {
    console.warn(`    getEventGUIDs failed for ${tournamentGUID.slice(0,8)}: ${err.message}`);
    return [];
  } finally {
...
async function scrapeFencer(fencer) {
  ...
  // Step 2 — Get event GUIDs for all tournaments
  const tourneyEventPairs = await batchAll(
    tournaments,
    async (t) => {
      const guids = await getEventGUIDs(t.id);
      return guids.map(g => ({
        eventGUID:      g,
  ...
  console.log(`  Checking ${allEvents.length} events for ${surname}...`);
